...
const StatCard = ({label, data}) => {
    ...
    return <Box bg={'Lighter grey'} px={20} mx={innerWidth < MOBILE_WIDTH ? 0 : 10}
                width={innerWidth < MOBILE_WIDTH ? 1 : 1 / 2} mb={10} sx={{height: '190px'}}>
        <h3>{label}</h3>
        ...
        {data?.global_national && <Comparison label={'NATIONAL'} reference={data.global_national} value={data.global}/>}
                {data?.global_region &&
                <Comparison label={'REGION'} reference={data.global_region} value={data.global}/>}
                {data?.global_dept &&
                <Comparison label={'DEPARTEMENT'} reference={data.global_dept} value={data.global}/>}
      </span>
        </Flex>
    </Box>;
};
...
const Print = () => {
    return <div className='no-print' style={{position: 'fixed', bottom: '10px', right: '10px'}}><Button
    ...
    return (
        <>
            ...
            <div style={{margin: '0 10px'}}>
                ...
                <p>La fragilité numérique est identifiée sur des critères liés à l'accès au numérique et sur le niveau
                    de compétences de chacun et chacun.</p>

                <p>L’indice de fragilité numérique des territoires a été produit dans le cadre de l’IncubO du SGAR
                    Occitanie avec le concours de l’ANSA en partenariat avec la Mednum, grâce au soutien du Fond de
                    transformation pour l’action publique.</p>
            </div>
            ...
            <Flex sx={{flexDirection: innerWidth < MOBILE_WIDTH ? 'column' : 'row'}}>
                ...
                {region && department && <>
                    <Box width={innerWidth < MOBILE_WIDTH ? 1 : 1 / 4} px={2}>
                        <CitySelect disabled department={department} region={region} onChange={setCity}/>
                    </Box>
                    <Box width={innerWidth < MOBILE_WIDTH ? 1 : 1 / 4} px={2}>
                        <NeighbourSelect disabled city={city} department={department} region={region} onChange={setDistrict}/>
                    </Box>
                </>
                }
            ...
            <h3 style={{margin: '10px 10px'}}>Vous regardez l'indice de fragilité numérique calculé au niveau
                : {address.toUpperCase()}</h3>
            ...
            <div style={{margin: '0 10px'}}>
                ...
                <p>Les données utilisées pour construire cet indicateur de fragilité numérique proviennent de bases
                    publiées en Open Data (data.gouv.fr, Insee ) :</p>

                <p>INSEE :</p>

                <ul>
                    <li><a href="https://www.insee.fr/fr/statistiques/4228434" target="_blank">Population en 2016
                        Recensement de la population - Base infracommunale (IRIS)</a></li>
                    <li><a href="https://www.insee.fr/fr/statistiques/4228428" target="_blank">Couples - Familles -
                        Ménages en 2016 - Recensement de la population - Base infracommunale (IRIS)</a></li>
                    <li><a href="https://www.insee.fr/fr/statistiques/4228430" target="_blank">Diplômes - Formation en
                        2016 - Recensement de la population - Base infracommunale (IRIS)</a></li>
                    <li><a href="https://www.insee.fr/fr/statistiques/4190004" target="_blank">Revenus et pauvreté des
                        ménages en 2016 - Dispositif Fichier localisé social et fiscal (Filosofi)</a></li>
                    <li><a href="https://www.insee.fr/fr/statistiques/4217503" target="_blank">Revenus, pauvreté et
                        niveau de vie en 2015 (IRIS) - Dispositif Fichier localisé social et fiscal (Filosofi)</a></li>
                </ul>
                ...
                <ul>
                    <li><a
                        href="https://www.data.gouv.fr/fr/datasets/le-marche-du-haut-et-tres-haut-debit-fixe-deploiements/"
                        target="_blank">Le marché du haut et très haut débit fixe (déploiements)</a></li>
                    <li><a href="https://www.data.gouv.fr/fr/datasets/mon-reseau-mobile/" target="_blank">Mon réseau
                        mobile</a></li>
                </ul>
            </div>
